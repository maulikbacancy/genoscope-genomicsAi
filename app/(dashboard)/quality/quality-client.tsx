    ...
    return (
      <div className="flex flex-col min-h-full">
        ...
        <div className="flex flex-col items-center justify-center flex-1 p-6">
          <BarChart3 size={32} className="mb-3" style={{ color: "var(--text-muted)" }} />
          <p style={{ color: "var(--text-secondary)" }}>No quality metrics yet</p>
          <p className="text-sm mt-1" style={{ color: "var(--text-muted)" }}>Upload samples to generate QC data</p>
        </div>
      </div>
    );
  }
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="card rounded-2xl p-6">
            <h3 className="font-semibold mb-4" style={{ color: "var(--text-primary)" }}>Coverage per Sample</h3>
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={coverageData}>
                <CartesianGrid strokeDasharray="3 3" stroke="var(--border)" />
                <XAxis dataKey="name" tick={{ fill: "var(--text-muted)", fontSize: 11 }} axisLine={false} tickLine={false} />
                <YAxis tick={{ fill: "var(--text-muted)", fontSize: 11 }} axisLine={false} tickLine={false} />
                <Tooltip contentStyle={{ background: "var(--bg-card-solid)", border: "1px solid var(--border)", borderRadius: "12px", color: "var(--text-primary)" }} />
          ...
          <div className="card rounded-2xl p-6">
            <h3 className="font-semibold mb-4" style={{ color: "var(--text-primary)" }}>QC Radar</h3>
            <ResponsiveContainer width="100%" height={220}>
        ...
        <div className="card rounded-2xl overflow-hidden">
          <div className="px-6 py-4 border-b" style={{ borderColor: "var(--border)" }}>
            <h3 className="font-semibold" style={{ color: "var(--text-primary)" }}>Sample QC Details</h3>
          </div>
          <table className="w-full">
            <thead>
              <tr style={{ borderBottom: "1px solid var(--border)" }}>
                {["Barcode", "Type", "Mean Coverage", "% ≥20x", "Ts/Tv", "Total Variants", "dbSNP %", "Status"].map((h) => (
                  <th key={h} className="px-4 py-3 text-left text-xs font-medium uppercase" style={{ color: "var(--text-muted)" }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {metrics.map((m, i) => {
                ...
                return (
                  <tr key={m.id} style={{ borderBottom: i < metrics.length - 1 ? "1px solid var(--border)" : "none" }}
                    ...
                    <td className="px-4 py-3 text-sm capitalize" style={{ color: "var(--text-secondary)" }}>{m.samples?.sample_type?.replace("_", " ") ?? "—"}</td>
                    <td className="px-4 py-3 text-sm font-mono" style={{ color: "var(--text-primary)" }}>{m.mean_coverage?.toFixed(1) ?? "—"}x</td>
                    <td className="px-4 py-3 text-sm font-mono" style={{ color: (m.percent_20x ?? 0) >= 90 ? "#34d399" : "#f43f5e" }}>
                      {m.percent_20x?.toFixed(1) ?? "—"}%
                    </td>
                    <td className="px-4 py-3 text-sm font-mono" style={{ color: "var(--text-primary)" }}>{m.ts_tv_ratio?.toFixed(3) ?? "—"}</td>
                    <td className="px-4 py-3 text-sm" style={{ color: "var(--text-primary)" }}>{m.total_variants?.toLocaleString() ?? "—"}</td>
                    <td className="px-4 py-3 text-sm font-mono" style={{ color: "var(--text-primary)" }}>{m.dbsnp_percent?.toFixed(1) ?? "—"}%</td>
