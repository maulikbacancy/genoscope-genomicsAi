        ...
        <div className="card rounded-2xl overflow-hidden">
          {!filtered.length ? (
            <div className="flex flex-col items-center justify-center py-16">
              ...
              <p className="mb-2" style={{ color: "var(--text-secondary)" }}>No reports generated yet</p>
              <p className="text-sm" style={{ color: "var(--text-muted)" }}>Generate reports from case detail pages</p>
            </div>
          ) : (
              ...
              <tbody>
                {filtered.map((r: any, i) => (
                  <tr key={r.id} style={{ borderBottom: i < filtered.length - 1 ? "1px solid var(--border)" : "none" }}
                    ...
                    <td className="px-6 py-4 text-sm max-w-xs truncate" style={{ color: "var(--text-secondary)" }}>{r.summary?.slice(0, 80)}...</td>
                    <td className="px-6 py-4 text-sm" style={{ color: "var(--text-secondary)" }}>{formatDate(r.created_at)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
